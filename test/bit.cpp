...
TEST(head_bit_idx, basic) {
  using hhxx::head_bit_idx;
  static_assert(head_bit_idx<char>() == 7, "");
  static_assert(head_bit_idx('c') == 7, "");
}
...
TEST(head_bit_flag, basic) {
  using hhxx::head_bit_flag;
  static_assert(head_bit_flag<int>() == INT_MIN, "");
}
...
TEST(test_head_bit, basic) {
  using hhxx::test_head_bit;
  EXPECT_FALSE(test_head_bit(0));
  EXPECT_TRUE(test_head_bit(INT_MIN));
}
...
TEST(set_head_bit, basic) {
  using hhxx::set_head_bit;
  EXPECT_EQ(INT_MIN, set_head_bit(0));
}
...
TEST(clear_head_bit, basic) {
  using hhxx::clear_head_bit;
  EXPECT_EQ(0, clear_head_bit(INT_MIN));
}
...
TEST(flip_head_bit, basic) {
  using hhxx::flip_head_bit;
  EXPECT_EQ(0, flip_head_bit(INT_MIN));
  EXPECT_EQ(INT_MIN, flip_head_bit(0));
}
